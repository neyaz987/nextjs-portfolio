...
export default function Button({
  ...
  return (
    ...
    >
      <span
        className=
          "absolute inset-0 z-0 h-full translate-y-9 bg-yellow-300 transition-transform  duration-300 ease-in-out group-hover:translate-y-0" ></span>
      <span className="relative flex items-center justify-center gap-2">
        {label} {showIcon && <LuArrowDownToLine className="inline-block" />}
      </span>
    </PrismicNextLink>
  );
}
